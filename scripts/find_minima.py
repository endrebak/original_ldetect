...
def precompute_values(covar_dict, partitions, search_start, search_end, last_breakpoint, next_breakpoint):
    ...
    to_preread, begin_from = files_to_preread(partitions, covar_dict, last_breakpoint)
    ...
    for i in range(begin_from, len(iter_partitions)):
...
from metric import find_breakpoint_loci, compute_metrics
from helpers import covar_files_map, files_to_preread, find_start_locus, find_end_locus, read_partitions, update_covar_and_loci
...
if __name__ == "__main__":
    ...
    metric_sum, nzero, nonzero = metrics
    print(metric_sum, nzero, nonzero)
    raise
    ...
    for breakpoint_index, (search_start, search_end, breakpoint, last_breakpoint, next_breakpoint) in enumerate(starts_ends.itertuples(index=False)):
        ...
        print("len(v)", len(v))
        print("len(h)", len(h))
        print("len(union)", len(set(v).intersection(set(v))))
